...
size_t get_encoded_key_size(gamal_key_t key) {
    ...
    if(!key->is_public) {
        ...
        size += EC_POINT_point2oct(init_group, key->Y, POINT_CONVERSION_COMPRESSED, NULL, 0, ctx) + 2;
    }
    BN_CTX_free(ctx);
    return size;
}
int encode_key(unsigned char *buff, int size, gamal_key_t key) {
    ...
    cur_ptr++;
    if (key->is_public) {
        size_data = EC_POINT_point2oct(init_group, key->Y, POINT_CONVERSION_COMPRESSED, NULL, 0, ctx);
    } else {
        size_data = (size_t) BN_num_bytes(key->secret);

    }

    write_size(cur_ptr, size_data);
    cur_ptr += 2;
